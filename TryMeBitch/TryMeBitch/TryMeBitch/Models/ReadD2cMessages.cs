    ...
    public class ReadD2cMessages
    {
        ...
        public static async Task<List<string>> ReadFromIoTHub(string[] args)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    await Task.Delay(100); // check every second

                    if (DateTime.UtcNow - lastActivity > idleTimeout)
                    {
                        Console.WriteLine("Exiting due to inactivity timeout.");
        ...
        private static async Task<List<string>> ReceiveMessagesFromDeviceAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                {
                    if (!(partitionEvent.Data.SystemProperties["iothub-connection-device-id"].ToString().Contains("GP02") && partitionEvent.Data.SystemProperties["iothub-connection-device-id"].ToString().Contains("DEV01"))) { continue; }
                    ;
